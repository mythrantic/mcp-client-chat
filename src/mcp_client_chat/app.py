...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Show Google/GCP fields if any Google provider is selected
        needs_gcp = (
        ...
        if needs_gcp:
            if current_provider == "google" or current_embed_provider == "google":
                gcp_api_key = st.text_input(
                    ...
                    help="Google API key for Gemini (required for google provider)",
                )
                if gcp_api_key:
            ...
            if current_provider in ("vertex-gemini", "vertex-claude"):
                gcp_project = st.text_input(
                    ...
                    help="Google Cloud project ID (required for Vertex AI providers)",
                )
                if gcp_project:
        ...
        if st.button("Initialize Client", type="primary"):
            ...
            if initialize_client(
                ...
                st.success(
                    f"✓ Client initialized with {st.session_state.get('llm_provider', 'default')} / {st.session_state.get('llm_model', 'default')}!"
                )
                ...
                with st.expander("Token Usage & Cost"):
                    ...
                    if usage.input_tokens:
                        ...
                        if usage.cache_read_tokens:
                            st.write(
                                f"  - Regular: {regular_input_tokens:,} (kr {input_cost:.4f})"
                            )
                            st.write(
                                f"  - Cached: {cached_input_tokens:,} (kr {cached_input_cost:.4f})"
                            )
                        else:
        ...
        if tool_calls and len(tool_calls) > 0:
            with st.expander(f"⚙️ Tool Calls ({len(tool_calls)})", expanded=False):
                for i, call in enumerate(tool_calls, 1):
                    ...
                    st.markdown(
                        f'<div class="tool-header">Tool {i}: {call["name"]}</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    with col1:
                        st.markdown(
                            '<div class="tool-section">', unsafe_allow_html=True
                        )
                        st.markdown(
                    ...
                    if thinking_container:
                        ...
                        if not thinking_sections or len(current_thinking) < 10:
                            thinking_expander = thinking_container.expander(
                                f"🧠 Model Reasoning {len(thinking_sections) + 1}"
    ...
    if prompt := st.chat_input("Type your message here..."):
        ...
        with st.chat_message("assistant"):
            ...
            if result.get("tool_calls"):
                with st.expander(
                    f"⚙️ Tool Calls ({len(result['tool_calls'])})", expanded=False
                ):
